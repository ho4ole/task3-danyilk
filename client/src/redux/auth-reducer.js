import {loginUser, registerUser, updateUserTimeRequest} from "../api/Api";
...
export const signUpUser = (data) => async (dispatch) => {
    ...
    dispatch(setUpAuth());
    dispatch(setUpLoginID(response._id));
}
...
export const logInUser = (data) => async (dispatch) => {
    let response = await loginUser(data);
     if (response.user) {
         if (response.user.status === "blocked") alert("You was Blocked!")
         else{updateUserTimeRequest(response.user._id).then(res => {console.log((res))}); dispatch(setUpAuth()); dispatch(setUpLoginID(response.user._id))}}
    }
